...
const arrText = [
    ...
    'Paham?<br>Ketika membaca tentang kesimpulan dari penelitian apapun, jangan lupa mengecek pernyataan mengenai hubungan sebab-akibat antar variabelnya, dan jika pernyataan itu memang ada, cek apakah partisipan penelitian tersebut dikelompokkan secara acak.',
    'Tanpa pengelompokkan yang acak, desain penelitian lainnya pasti korelasional atau kuasi-eksperimen, yang tidak dapat digunakan oleh peneliti atau siapapun untuk mengklaim adanya hubungan sebab-akibat antar variabel.'
]

const indexPar = Math.round(arrText.length / 10)
let index = 0
let outP = indexPar - 1
...
function nextC (obj, index) {
    ...
    if (index <= arrText.length) {
        ...
        if (index == outP) {
            $('.modal').show('fade')
            $('.modal').find('.close, .yes-c, .no-c').on('click', function (e) {
                e.preventDefault()
        ...
        window.location.replace(base_url + 'main/link')
}
...
function nextD (obj, index) {
    ...
    if (index <= arrText.length) {
        ...
        if (index == outP) {
            $('.modal').show('fade')
            $('.modal').find('.close, .yes-d, .no-d').on('click', function (e) {
                e.preventDefault()
...
$('.yes-a, .yes-b, .yes-c, .yes-d').on('click', function (e) {
    e.preventDefault()
    ...
    $.ajax({
        url: base_url + 'card/setYes/' + user_id + '/' + card,
        method: 'POST',
        cache: false,
        success: function () {}
    })
...
$('.no-a, .no-b, .no-c, .no-d').on('click', function (e) {
    e.preventDefault()
    ...
    $.ajax({
        url: base_url + 'card/setNo/' + user_id + '/' + card,
        method: 'POST',
        cache: false,
        success: function () {}
    })
